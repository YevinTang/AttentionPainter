...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, (samples, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        for idx in range(samples.shape[0]):
            ...
            # density = cv2.medianBlur(gradient_magnitude, 14)
        ...
        with torch.cuda.amp.autocast():
            loss, _, loss_mse, loss_density = model.density_loss(samples, masks)
        ...
        loss_mse_value_reduce = misc.all_reduce_mean(loss_mse_value)
        ...
        if log_writer is not None and (data_iter_step + 1) % accum_iter == 0:
            ...
            log_writer.add_scalar('loss_mse', loss_mse_value_reduce, epoch_1000x)
